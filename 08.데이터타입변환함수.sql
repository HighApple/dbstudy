...
2. 테이블이 필요 없는 조회(DQL)의 경우에도 FROM 절까지 작성해야 하는데 이 때 사용한다.
...
TO_NUMBER(문자)
*/
SELECT '100', TO_NUMBER('100') -- 문자로서는 왼쪽, 숫자로서는 오른쪽
  FROM DUAL;  
  
  SELECT *
  FROM DEPARTMENT_T
 WHERE DEPT_NO = '1';
-- 숫자인데도 '1' 이라도 쳐도 실행됨. 오라클이 이렇게 바뀐 뒤에 실행된다.
...
SELECT SYSDATE
     , SYSTIMESTAMP
     , TO_CHAR(SYSDATE, 'YYYY-MM-DD AM HH:MI:SS') -- 원하는 날짜 형식을 만들 땐 TO_CHAR
     , TO_CHAR(SYSTIMESTAMP, 'YYYY-MM-DD AM HH:MI:SS.FF3')
  FROM DUAL;

--TO_DATE는 문자를 날짜로 전환. TO_CHAR(DATE)는 날자를 문자로 전환
