...
다중조인 - 테이블 3개 나란히 놓는다. ON은 WHERE 로 바꾼다. 두번째 ON은 WHERE로 바꾼다. 두번째 ON 은 AND로 바꾼다. ]
WHERE 로건문이 있는 경우, 조인의 WHERE 써주고 그냥 AND로 같이 묶어준다.
*/
-- 외부 조인 확인을 위한 데이터 입력
...
SELECT E.EMPLOYEE_ID, E.FIRST_NAME, D.DEPARTMENT_ID, L.LOCATION_ID, L.STREET_ADDRESS
